...
import com.prashant.auth.auth_app_backend.dtos.RefreshTokenRequest;
...
import com.prashant.auth.auth_app_backend.security.CookieService;
...
//        use cookie Service to attach the refreshtoken with the cookie in response
...
//    access and refresh token renew api logic
    ...
    HttpServletRequest request
        ...
        UUID userId = jwtService.getUserIdByToken(refreshToken);
        ...
        if(!storedRefreshToken.getUser().getId().equals(userId)){
          throw new BadCredentialsException("Refresh token does not belong to the user - token subject mismatch");
        }

//        rotate refresh token logic
...
//    this method will read the refresh token from the request body
...
//       4.authorisation = Bearer <Token> like accessToken
